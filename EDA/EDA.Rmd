...
We will use these criteria to focus our approach in two steps. First we want to select the best industries in terms of returns over the whole period considered. We arbirtrarily choose ten industries to reduce the scope of our selection. 
Then, among each industry we will select the top ten best stocks in terms of alpha (i.e. the most overperforming ones).
We )
...
Another approach to get better insights onwhich stocks would be most interesting is to focus on specific industries. We'll create a ranking system similar to what we previously did on stocks, aggregating however on industrial codes.

```{r}
top_industries <- merged_data %>% 
  group_by(siccd) %>% 
  mutate(nb_of_ent = n()) %>% 
  ungroup() %>% 
  filter(nb_of_ent > 50) %>% 
